...
class RNNNumpy:
    ...
    def backprop(self, xs, ys, truncation=0):
        """
        Backprop through time to compute gradients of softmax loss function wrt parameters, using one training example;
        this is just standard backprop applied to the unrolled computational graph of RNN; as a side
        effect, self.h is updated by the forward computation.
        ...
        prev_h = self.h  # useful for later
        ...
        y_hats = np.empty((T, self.data_size))  # predicted probs

        L = 0
        for t in range(T):
            a = self.b + self.U[:, xs[t]] + np.dot(self.W,
                                                   self.h)  # (10.8); note that indexing by xs[t] is equiv to multiplication
            h = np.tanh(a)  # (10.9)
            o = self.c + np.dot(self.V, h)  # (10.10)
        ...
        dos = np.copy(y_hats)
        for t in range(T):  # (10.18), for all time steps
            dos[t][ys[t]] -= 1

        # BPTT; only perform T-truncation time steps of computations
        dh = np.dot(self.V.T, dos[-1])  # (10.19) for last time step
        dhs = [dh]  # will contain `trunc` vectors in total
        for t in range(T - 2, truncation - 1, -1):
            dh = np.dot(self.W.T, (1 - hs[t + 1] ** 2) * dh) + np.dot(self.V.T, dos[t])  # (10.21)
            dhs.append(dh)
        ...
        if truncation == 0:  # in case we BPTT all the way to the beginning
            dW = np.dot(((1 - hs[truncation:] ** 2) * dhs[truncation:]).T,
                        np.vstack((prev_h, hs))[:T - truncation])  # (10.26)
        else:
